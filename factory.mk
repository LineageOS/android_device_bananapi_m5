...
ifneq ("$(wildcard $(FACTORY_PATH)/bootloader.img)","")
	$(hide) $(call aml-copy-install-file, $(FACTORY_PATH)/bootloader.img, u-boot.bin)
else ifeq ($(WITH_CONSOLE_BL),true)
	$(hide) $(call aml-copy-install-file, $(FACTORY_PATH)/bootloader-console.img, u-boot.bin)
else
	$(error "no bootloader.img found in $(FACTORY_PATH)")
endif
...
ifneq ("$(wildcard $(FACTORY_PATH)/bootloader.img)","")
	$(hide) $(call aml-copy-upgrade-file, $(FACTORY_PATH)/bootloader.img, u-boot.bin)
else ifeq ($(WITH_CONSOLE_BL),true)
	$(hide) $(call aml-copy-install-file, $(FACTORY_PATH)/bootloader-console.img, u-boot.bin)
else
	$(error "no bootloader.img found in $(FACTORY_PATH)")
